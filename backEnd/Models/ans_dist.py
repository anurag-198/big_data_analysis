...
lines = hq.sql (""" select courseName,lmsUserId,createDateTime,attempts,
chapterTitle,moduleSysName,moduleTitle,success from logdata 
where eventName='problem_check' and eventSource='server' and success is not NULL""")
#(problem_id,userid)(DateTimeStamp,coursename,chaptertitle,moduletitle,attempts,sucess)

#for line in lines.collect() :
#	print line
maplvl1=lines.map(lambda p: ((p[5],p[1]),(p[2],p[0],p[4],p[6],p[3],p[7])))
